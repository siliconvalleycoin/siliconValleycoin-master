    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        switch(rcp.txnTypeInd)
        {
            ...
            case TXT_SDC_TO_ANON:
                formatted.append(tr("<b>%1</b> to SiliconValley %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SDC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> SiliconValley, ring size %2 to SiliconValley %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SDC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_SDC:
                formatted.append(tr("<b>%1</b> SiliconValley, ring size %2 to SVC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SDC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
...
QVariantMap ShadowBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with SiliconValley prefix.");
            return result;
        }
