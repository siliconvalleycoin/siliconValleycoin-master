// Copyright (c) 2015 The SiliconValleyopers
...
int ExtractBip32InfoV(std::vector<unsigned char> &vchKey, Object &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0)
        keyInfo.push_back(Pair("type", "SiliconValleyded secret key"));
    else
...
int ExtractBip32InfoP(std::vector<unsigned char> &vchKey, Object &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0)
        keyInfo.push_back(Pair("type", "SiliconValleyded public key"));
    else
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY_BTC)[0], 4) == 0)
        ...
        if (eKey58.Set58(sInKey.c_str()) == 0)
        {
            ...
            } else
            {
                if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                    ...
                    throw std::runtime_error("Import failed - Key must begin with SiliconValleyx.");
            };
