...
# for boost 1.37, add -mt to the boost SiliconValley
...
build_macosx64 {
    QMAKE_TARGET_BUNDLE_PREFIX = co.SiliconValley
    BOOST_LIB_SUFFIX=-mt
...
# also add new translations to SiliconValley.qrc under translations/
...
macx:TARGET = "SiliconValley"
macx:QMAKE_CFLAGS_THREAD += -pthread
macx:QMAKE_LFLAGS_THREAD += -pthread
macx:QMAKE_CXXFLAGS_THREAD += -pthread

# Set SiliconValley and includes at end, to use platform-defined defaults if not overridden
...
contains(RELEASE, 1) {
    !windows:!macx {
        # Linux: turn dynamic linking back on for c/c++ runtime SiliconValley
        LIBS += -Wl,-Bdynamic
    }
}
